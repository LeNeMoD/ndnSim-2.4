...
FibManager::FibManager(Fib& fib, const FaceTable& faceTable,
		...
		NfdManagerBase(dispatcher, authenticator, "fib"), m_fib(fib), m_faceTable(
				faceTable) {
	...
	registerStatusDatasetHandler("list",
			bind(&FibManager::listEntries, this, _1, _2, _3));
}

void FibManager::addNextHop(const Name& topPrefix, const Interest& interest,
...
//				<< " timeAtFuturePosition: " << timeAtFuturePosition);
		...
		return done(
				ControlResponse(200, "Success").setBody(
						parameters.wireEncode()));
	} else {
		NFD_LOG_INFO(
...
void FibManager::removeNextHop(const Name& topPrefix, const Interest& interest,
		ControlParameters parameters,
		const ndn::mgmt::CommandContinuation& done) {
	setFaceForSelfRegistration(interest, parameters);

	NFD_LOG_TRACE(
			"remove-nexthop prefix: " << parameters.getName() << " faceid: " << parameters.getFaceId());
	...
	if (face != nullptr) {
		...
		if (entry != nullptr) {
			entry->removeNextHop(*face, mac);
			NFD_LOG_DEBUG(
					"remove-nexthop result: OK prefix: " << parameters.getName() << " faceid: " << parameters.getFaceId());
...
void FibManager::listEntries(const Name& topPrefix, const Interest& interest,
	...
	for (const auto& entry : m_fib) {
		...
		context.append(
				ndn::nfd::FibEntry().setPrefix(entry.getPrefix()).setNextHopRecords(
						std::begin(nexthops), std::end(nexthops)).wireEncode());
	}
	context.end();
}
...
void FibManager::setFaceForSelfRegistration(const Interest& request,
	...
	if (isSelfRegistration) {
		shared_ptr<lp::IncomingFaceIdTag> incomingFaceIdTag = request.getTag<
				lp::IncomingFaceIdTag>();
